...
app.config['MAX_CONTENT_LENGTH'] = 16 * 1024 * \
    1024  # Limit file upload size (e.g., 16MB)
...
class CapaIssue(db.Model):
    ...
    # e.g., 'Open', 'RCA Pending', 'Action Pending', 'Evidence Pending', 'Closed'
...
class Evidence(db.Model):
    ...
    evidence_photo_path = db.Column(
        db.String(300), nullable=False)  # Path to evidence photo
...
def new_capa():
    if request.method == 'POST':
        ...
        try:
            ...
            flash(
                f'New CAPA issue (ID: {new_issue.capa_id}) created successfully! Triggering AI Root Cause Analysis...', 'success')
            ...
            try:
                ...
                print(
                    f"Error triggering AI RCA for CAPA ID {new_issue.capa_id}: {ai_error}")
                flash(
                    f'Issue {new_issue.capa_id} created, but AI Root Cause Analysis failed: {ai_error}', 'warning')
            # ------------------------------------
